...
export function useSortedCandles(candles, category, sort) {
    const sortedCandles = useMemo(() => {
        ...
        if (category) {
            if (category === 'All') {
                tmpSortedCandles = candles;
            } else {
                tmpSortedCandles = candles.filter(candle => candle.category['title'].includes(category));
            }
        }
        if (sort) {
...
export function useCandles(candles, category, query, page, sort, limit) {
    ...
    const searchedCategoryCandles = useMemo(() => {
        let searchedCandles = sortedCandles.filter(candle => candle.title.toLowerCase().includes(query.toLowerCase()));
